...
class DataStore:
    ...
    def list_accounts(self) -> list[Optional[str]]:
        ...
        entries = next(walk(self.data_dir), None)
        if entries is None:
            return accounts
        for account_id in entries[1]:
    ...
    def list_devices(self, account_id) -> list[Optional[str]]:
        ...
        entries = next(walk(self.account_devices_dir(account_id)), None)
        if entries is None:
            return devices
        for device_id in entries[1]:
    ...
    def list_poweron_devices(self) -> list[str]:
        ...
        """
        devices: list[str] = []
        entries = next(walk(self.poweron_devices_dir()), None)
        if entries is None:
            return devices
        for device_id in entries[1]:
            devices.append(device_id)
